 ...
 * Created 2020-04-23 18:56, by config.py
 */
...
int apply_config(struct hub_config* config, const char* key, const char* data, int line_count)
{
	...
	if (!strcmp(key, "tls_ciphersuite"))
	{
		...
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"tls_ciphersuite\" (string), default=\"ECDH+AESGCM:DH+AESGCM:ECDH+AES256:DH+AES256:ECDH+AES128:DH+AES:ECDH+3DES:DH+3DES:RSA+AESGCM:RSA+AES:RSA+3DES:!aNULL:!MD5:!DSS\"");
			return -1;
		}
		return 0;
	}
...
void dump_config(const struct hub_config* config, FILE* stream, int ignore_defaults)
{
	...
	if (!ignore_defaults || strcmp(config->tls_ciphersuite, "ECDH+AESGCM:DH+AESGCM:ECDH+AES256:DH+AES256:ECDH+AES128:DH+AES:ECDH+3DES:DH+3DES:RSA+AESGCM:RSA+AES:RSA+3DES:!aNULL:!MD5:!DSS") != 0)
		fprintf(stream, "tls_ciphersuite = \"%s\"\n", config->tls_ciphersuite);
