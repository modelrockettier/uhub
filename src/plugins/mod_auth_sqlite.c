...
static plugin_st register_user(struct plugin_handle* plugin, struct auth_info* userinfo)
{
	...
	if (rc <= 0)
	{
		LOG_ERROR("Unable to add user \"%s\": %s", nick, sqlite3_errstr(-rc));
		return fail;
	}
	return st_allow;
}
...
static plugin_st update_user(struct plugin_handle* plugin, struct auth_info* userinfo)
{
	...
	if (rc <= 0)
	{
		LOG_ERROR("Unable to update user \"%s\": %s", nick, sqlite3_errstr(-rc));
		return fail;
	}
	else if (rc > 1)
	{
		LOG_WARN("Updated %d users! \"%s\"", rc, nick);
	}
	return st_allow;
}
...
static plugin_st delete_user(struct plugin_handle* plugin, struct auth_info* userinfo)
{
	...
	if (rc <= 0)
	{
		LOG_ERROR("Unable to delete user \"%s\": %s", nick, sqlite3_errstr(-rc));
		return fail;
	}
	else if (rc > 1)
	{
		LOG_WARN("Deleted %d users! \"%s\"", rc, nick);
	}
	return st_allow;
}
